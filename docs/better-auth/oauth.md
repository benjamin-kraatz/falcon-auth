...
- **Scopes** define what a client application _requests_ on behalf of a user. They are usually coarse-grained labels included in an access token.
...
oauthProvider({
  ...
  cachedTrustedClients: new Set(["internal-dashboard", "mobile-app"]),
});
```
...
```ts title="auth.ts"
oauthProvider({
  validAudiences: ["https://api.example.com", "https://api.example.com/mcp"],
});
```
